...
export default class Util {
     ...
     * @param {Date|null|numeric} stale     Expiration of CoT - if null now+20s is used. Alternative an integer representing the ms offset
     *
     * @returns {Object}
     */
    static cot_date(time, start, stale) {
        ...
        if (!stale) {
            stale = new Date(+new Date(now) + 20 * 1000).toISOString();
        } else if (!isNaN(parseInt(stale))) {
            stale = new Date(+new Date(now) + stale).toISOString();
        }
