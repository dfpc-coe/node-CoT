...
test(`DataPackage CoT Writing`, async () => {
    ...
    assert.equal(pkg_parse.version, '2');
    assert.ok(pkg_parse.path);
    assert.deepEqual(pkg_parse.settings, {
        uid: 'top-123',
        name: 'Custom 123'
    });

    assert.deepEqual(pkg_parse.contents, [
        {
...
test(`DataPackage CoT Parsing: QuickPic.zip`, async () => {
    ...
    assert.deepEqual(attachments.get('3b758d3c-5b7a-4fba-a0dc-bbde18e895b5'), [{
        _attributes: { ignore: false, zipEntry: 'e63e920689815c961ec9d873c83f08a6/20240702_144514.jpg' },
        Parameter: { _attributes: { name: 'uid', value: '3b758d3c-5b7a-4fba-a0dc-bbde18e895b5' } }
    }]);
...
test(`DataPackage CoT Parsing: addFile,getFile`, async () => {
    ...
    assert.equal(JSON.parse(buff.toString()).name, '@tak-ps/node-cot');
...
test(`DataPackage CoT Parsing: AttachmentInManifest.zip`, async () => {
    ...
    assert.deepEqual(attachments.get('c7f90966-f048-41fd-8951-70cd9a380cd2'), [{
        _attributes: { ignore: false, zipEntry: '6988443373b26e519cfd1096665b8eaa/1000001544.jpg' },
        Parameter: { _attributes: { name: 'uid', value: 'c7f90966-f048-41fd-8951-70cd9a380cd2' } }
    }]);
...
test(`DataPackage File Parsing: COMileposts.zip`, async () => {
    ...
    assert.ok(files.has('86771c6fd3635ec4d66c3fd9501f23c1/CO Mileposts.kmz'));
...
test(`DataPackage File Parsing: Iconset-FalconView.zip (strict: false)`, async () => {
    ...
    assert.deepEqual(pkg.contents.length, 490);
    ...
    const files = await pkg.files();
    assert.equal(files.size, 490)

    assert.ok(files.has('iconset.xml'));

    assert.equal(pkg.isMissionArchive(), false)
...
test(`MissionArchive: Testing Export`, async () => {
    ...
    assert.deepEqual(pkg.contents.length, 0);
